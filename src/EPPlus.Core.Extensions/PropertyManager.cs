...
namespace EPPlus.Core.Extensions
{
    ...
    public class PropertyManager<T>
    {
        ...
        public PropertyManager(IEnumerable<PropertyByName<T>> properties)
        {
            ...
            var poz = 1;
            foreach (PropertyByName<T> propertyByName in properties)
            {
        ...
        public void WriteToXlsx(ExcelWorksheet worksheet, int row, int cellOffset = 0)
        {
            ...
            foreach (PropertyByName<T> prop in _properties.Values)
            {
                ExcelRange cell = worksheet.Cells[row, prop.PropertyOrderPosition + cellOffset];
        ...
        {
            foreach (PropertyByName<T> caption in _properties.Values)
            {
                ExcelRange cell = worksheet.Cells[row, caption.PropertyOrderPosition + cellOffset];
                cell.Value = caption;
                setStyle(cell.Style);
            }
        }
